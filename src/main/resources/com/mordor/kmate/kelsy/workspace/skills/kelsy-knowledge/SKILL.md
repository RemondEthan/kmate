...
`原文` 必填：抄用户写下这条待办时的原句（可多句），不要改写成标题、不要补事实。右侧页卡靠它对照秘书的回答。
...
齐了：先写会议/决定卡（「待办：」一行保留纪要），再为每条写 `knowledge/todos/`（`状态=open`，`- 原文：` 用会上那条待办的原句，可选 `- 来源：knowledge/meetings/…`）。用户说没有待办：只写会议/决定卡。不对旧会议做回填。
...
- 只用刚读到的原文，改写可以，事实不能加。待办卡对照 `- 原文：` 字段，不要只复述标题。
